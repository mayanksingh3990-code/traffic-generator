...
async function main() {
    console.log('[*] Bot started - waiting for jobs...');
    
    while (true) {
        try {
            const job = await getJob();
            if (job.sleep) {
                console.log(`[*] No job available, sleeping for ${job.sleep}ms...`);
            ...
            const interval = job.interval || 5000;
            console.log(`[*] Batch complete, waiting ${interval}ms before next batch...`);
